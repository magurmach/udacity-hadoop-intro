...
def mapper():
    ...
    for line in reader:
        body = str(line[4])
        numberOfPeriod = body.count('.')
        numberOfExclamation = body.count('!')
        numberOfQuestion = body.count('?')
        list = ['.', '!', '?']
        if numberOfExclamation + numberOfPeriod + numberOfQuestion > 1:
            continue

        lastChar = body[-1]

        if numberOfExclamation + numberOfPeriod + numberOfQuestion == 1 and not list.__contains__(lastChar):
            continue
